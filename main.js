...
function renderColumns(data) {
    ...
    data.elections.forEach((election, colIdx) => {
        ...
        election.parties.forEach((party, partyIdx) => {
            ...
            const logo = PartyData.getLogo(party.id, election.year);
            if (logo) {
...
function setupOptimizeModal(data, rerender) {
...
let sharedGroups = [];
let electionData = {
    althingi: { data: null, order: null },
    borgarstjorn: { data: null, order: null }
};
...
function rerenderAll() {
    const { data } = electionData[currentElectionType];
    renderColumns(data);
